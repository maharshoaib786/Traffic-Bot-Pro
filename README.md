...
The current stable release is **v1.0.0**. See [CHANGELOG.md](CHANGELOG.md) for a detailed list of features included in this first public version.
...
The project ships with a lightweight build pipeline so you can publish compiled artifacts to GitHub releases.
...
Artifacts are written to `dist/`. Use `make clean` to remove build outputs.

### Publishing to GitHub

1. Ensure your changes are committed and pushed.
2. Tag the commit using the semantic `vX.Y.Z` pattern, for example:
...
3. The GitHub Actions workflow in `.github/workflows/publish.yml` compiles the project, uploads the distribution as an artifact, and attaches it to the release automatically.
